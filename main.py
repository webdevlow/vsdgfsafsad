...
async def start(message: types.Message):
	...
	if result[0] == 0:
		...
		await message.answer('Ты получил пизду ублюдок')
...
async def h(message: types.Message, state: FSMContext):
	...
	if result[0] == 0:
		...
		await message.answer('Долбаёб ты получил пизду в боте еблан.')
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 0:
					...
					await state.finish()
					await bot.send_message(message.text, 'Ты получил пизду от администрацией.')
				else:
